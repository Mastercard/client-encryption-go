...
func TestParseJWEObject(t *testing.T) {
	jweObject, err := jwe.ParseJWEObject(encryptedPayloadGcm)
...
func TestDecrypt_ShouldReturnDecryptedPayload_WhenPayloadIsGcmEncrypted(t *testing.T) {
	jweObject, err := jwe.ParseJWEObject(encryptedPayloadGcm)
	assert.Nil(t, err)
...
func TestJWEObjectSerialize(t *testing.T) {
	...
	assert.Equal(t, encryptedPayloadGcm, serializedData)
}
